...
class AbsoluteDriver implements DriverInterface, LocalPaymentDriverInterface
{
    ...
    protected function getAccessToken(): string
    {
        if (empty($this->accessToken)) {
            if ($token = Tokens::getValue(self::class)) {
                $this->accessToken = $token;
            } elseif ($this->accessToken = $this->getToken()) {
                Tokens::setValue(self::class, $this->accessToken, Carbon::now()->addMinutes(55));
            }
        }
    ...
    protected function getToken(): string
    {
        ...
        try {
            ...
            throw new AbsoluteDriverException(
                ...
                AbsoluteDriverException::DEFAULT_CODE,
                $throwable
            );
        }
    ...
    public function calculate(array $data): CalculatedInterface
    {
        ...
        if (self::isLife($data) || self::isPropertyAndLife($data)) {
            $body = [
                ...
                'birthday' => Carbon::parse(Arr::get($data, 'objects.life.birthDate'))->format(
                    'Y-m-d\TH:i:s'
                ),
            ];
    ...
    {
        $body = [
            'date_begin' => Carbon::parse(Arr::get($data, 'activeFrom'))->format('Y-m-d'),
            'agr_credit_number' => Arr::get($data, 'mortgageAgreementNumber'),
            'agr_credit_date_conc' => Carbon::parse(Arr::get($data, 'activeTo'))->format('Y-m-d'),
            'limit_sum' => Arr::get($data, 'remainingDebt'),
            'policy_holder' => [
                ...
                'birthday' => Carbon::parse(
                    Arr::get($data, 'subject.birthDate')
                )->format('Y-m-d\TH:i:s'),
                'address' => [
                    [
                        ...
                        'fias_id' => Arr::get($data, 'objects.property.fiasId', ''),
                    ],
                    [
                ...
                'document' => [
                    ...
                    'issue_date' => Carbon::parse(
                        Arr::get($data, 'subject.docIssueDate')
                    )->format('Y-m-d\TH:i:s'),
